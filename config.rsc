# jun/23/2025 23:13:06 by RouterOS 6.49.18
...
add interface=br-lan name=vlan20-servers vlan-id=20
add interface=br-lan name=vlan30-users vlan-id=30
...
add address-pool=mgmt-pool disabled=no interface=vlan10-mgmt name=\
    dhcp-mgmt
add address-pool=servers-pool disabled=no interface=vlan20-servers name=\
    dhcp-servers
add address-pool=users-pool disabled=no interface=vlan30-users name=\
    dhcp-users
...
add address=10.10.10.1/24 comment="gateway for mgmt" interface=vlan10-mgmt \
    network=10.10.10.0
add address=10.10.20.1/24 comment="gateway for servers" interface=\
    vlan20-servers network=10.10.20.0
add address=10.10.30.1/24 comment="gateway for users" interface=\
    vlan30-users network=10.10.30.0
